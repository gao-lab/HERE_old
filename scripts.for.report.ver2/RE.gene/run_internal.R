...
## generate table of RE-targeted genes and plot counts per stage
{
    ...
    . + labs(x="", y="# RE-targeted genes", fill="within-stage\nediting style") ->.;
    . + theme_pubr(base_size=10) ->.;
    . + theme(axis.text.x=element_text(angle=45, hjust=1), legend.position="right") ->.;
    ggsave.A4(
        filename="./report.ver2/210215-sixth-dataset/201221-fifth-phenotype-collection/all.normal.samples/RE.gene/RE.gene.count.per.stage.png",
        plot=.,
...
## generate sankey plot
{
    ...
    foreach(
        ...
        temp.part=c(
            ...
            temp.dt[is.na(Annotation.class.pasted.description.mixed) == TRUE, Annotation.class.pasted.description.mixed:="not RE-targeted"]
            temp.dt[, part:=temp.part]
        } ->.; rbindlist(.) ->.;
    .[, part.ordered:=factor(part, levels=unique(part))]
    ## re-prettify Annotation
    .[, Annotation.class.pasted.description.mixed.ordered:=factor(c("exonic"="always exonic\n(mostly 3'-UTR)", "intronic"="always intronic", "mixed"="mixed", "exonic;intronic"="mixed", "exonic;mixed"="mixed", "intronic;mixed"="mixed", "exonic;intronic;mixed"="mixed", "not RE-targeted"="not RE-targeted")[Annotation.class.pasted.description.mixed], levels=c("always exonic\n(mostly 3'-UTR)", "always intronic", "mixed", "not RE-targeted"))] 
    ## prettify stage
    temp.stage.dt <- data.table(read_excel("./manuscript/table_for_processing.xlsx", sheet="stage.properties"))
    merge(x=., y=temp.stage.dt, by.x="stage", by.y="stage", all.x=TRUE, all.y=FALSE)[, stage.description.ordered:=factor(stage.description, levels=temp.stage.dt[, stage.description])][, before.or.after.8.cell.ordered:=factor(before.or.after.8.cell, levels=unique(temp.stage.dt[, before.or.after.8.cell]))] ->.;    
    ## make the plot
    ...
    ggsave.A4(
        filename="./report.ver2/210215-sixth-dataset/201221-fifth-phenotype-collection/all.normal.samples/RE.gene/RE.gene.sankey.png",
        plot=.,
