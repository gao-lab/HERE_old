...
BLCAP.example.ggplot <- for.plot.dt %>%
    {
        ggplot(., aes(x=paste(sep="", CHROM, ":", POS), y=SAMPLE, fill=cut(x=AF, breaks=c(0, 0.1, 0.2, 0.4, 1)))) +
            ...
            geom_richtext(data=data.table(CHROM="chr20", POS=37519170, SAMPLE=0, text=c("<span style='font-family:monospace'>BLCAP</span>", "<span style='font-family:monospace'>--GCAATACAT-></span>", "<span style='font-family:monospace'><-CGUUAUGUA--</span>", "<span style='font-family:monospace'><-CGUU<span style='color:#ff0000'>G</span>UGUA--</span>", "<span style='font-family:monospace'><--C--Y--M---</span>", "<span style='font-family:monospace'><--C--<span style='color:#ff0000'>C</span>--M---</span>"), stage.description.ordered=factor(c("gene", "genomic DNA", "unedited RNA", "edited RNA", "unedited AA", "edited AA"), levels=levels(.[, stage.description.ordered])), AF=as.numeric(NA)), mapping=aes(x=paste(sep="", CHROM, ":", POS), y=SAMPLE, label=text), fill=NA, label.size=0) + scale_color_manual(values=c("black", "red")) + guides(color="none") 
    }
ggsave.A4(filename="./report.ver2/210215-sixth-dataset/201221-fifth-phenotype-collection/all.normal.samples/BLCAP.example.png", plot=BLCAP.example.ggplot, width.r=0.4, height.r=0.5)
