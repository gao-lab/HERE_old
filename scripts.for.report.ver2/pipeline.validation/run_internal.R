...

{
    ...
    .[, positive.type:=c("TP", "FP")[(is.DNA.detected == TRUE) + 1]]
    ## start plotting
    ggplot(., aes(x=positive.type, y=log10(count+0.1), fill=positive.type)) ->.;
    ## plot boxplots
    . + geom_boxplot() ->.;
    ## add zero line
    . + geom_hline(yintercept=log10(0+0.1), color="red", linetype="dashed") ->.;
    ## facet
    . + facet_wrap(~RNA.edit.type, scales="free", nrow=1) ->.;
    ## add theme
    . + theme_pubr() ->.;
    . + theme(axis.text.x=element_blank()) ->.;
    . + labs(x="validation result", y="log10(# \"edits\" + 0.1) per sample", fill="") ->.;
    ## save image
    ggsave.A4(
        filename="./report.ver2/pipeline.validation/210203-GSE144296.A375/validation.png",
        plot=.,
        width.r=0.35, height.r=0.3)
}
