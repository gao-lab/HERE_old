...
normal.edits.dt <- fread("./result/A02_4__check_fine_recurrence_profile_for_a_subset_of_samples/210215-sixth-dataset/201221-fifth-phenotype-collection/all.normal.samples/subset.dt.txt.gz")
...
##subset normal.edits.dt
normal.edits.subset.recoding.only.dt <- merge(
    x=normal.edits.dt, y=normal.recoding.edits.dt,
    by=c("CHROM", "POS"),
    all.x=FALSE, all.y=TRUE)

fwrite(normal.edits.subset.recoding.only.dt, "./report.ver2/210215-sixth-dataset/201221-fifth-phenotype-collection/all.normal.samples/normal.edits.subset.recoding.only.dt.csv.gz")
