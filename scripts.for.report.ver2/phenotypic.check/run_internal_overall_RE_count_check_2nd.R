...

{
    ...
    normal.RE.and.gene.count.info.dt[gse %in% c("GSE101571", "GSE133854", "GSE95477")] ->.;
    ...
    ## start plotting
    common.option <- function(temp.x){temp.x + facet_wrap(~stage.description.ordered, scales="free", nrow=1) + theme_pubr(base_size=11) + scale_y_continuous(expand=expansion(mult=c(0.1, 0.2)))}
    (ggplot(.[gse=="GSE95477"], aes(x=final.label.reordered, y=count.RE)) + geom_boxplot() + stat_compare_means(comparisons = list(c("old", "young")), method="wilcox.test", method.args=list(alternative="less"))  + ggtitle("GSE95477") + labs(x="", y="# RE-matching edits")) %>% common.option -> temp.GSE95477.ggplot;
    (ggplot(.[gse=="GSE133854"], aes(x=final.label.reordered, y=count.RE)) + geom_boxplot() + stat_compare_means(comparisons = list(c("PG", "BI"), c("AG", "BI")), method="wilcox.test", method.args=list(alternative="less")) + ggtitle("GSE133854") + labs(x="", y="# RE-matching edits")) %>% common.option -> temp.GSE133854.ggplot;
    ...
    ggarrange(
        plotlist=list(
            ggarrange(plotlist=list(temp.GSE95477.ggplot, temp.GSE101571.ggplot), nrow=1, widths=c(1,1), align="h"),
...
##    . + ggtitle("All 2-cells") ->.;
    ## save image
    ggsave.A4(
        filename="./report.ver2/210215-sixth-dataset/201221-fifth-phenotype-collection/total.samples/normal.valid.gene.only.RE.count.histogram.three.datasets.png",
        plot=.,
